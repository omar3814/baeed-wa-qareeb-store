<!-- === Code for src/lib/components/ProductDetailModal.svelte (Vertical Align via Padding - FINAL CLEAN) === -->
<script lang="ts">
	...
	function calculateVerticalPadding() {
		if (!modalContentElement || !position) { calculatedPaddingTop = `${buffer}px`; return; };
		const modalRect = modalContentElement.getBoundingClientRect();
        const modalHeight = modalRect.height;
		const vh = window.innerHeight;
        let potentialPaddingTop = position.top + (position.height / 2) - (modalHeight / 2); // Aim for card vertical center

        if (potentialPaddingTop < buffer) { potentialPaddingTop = buffer; }
        if (potentialPaddingTop + modalHeight > vh - buffer) {
            potentialPaddingTop = vh - modalHeight - buffer;
             if (potentialPaddingTop < buffer) { potentialPaddingTop = buffer; }
        }
		calculatedPaddingTop = `${Math.max(buffer, Math.round(potentialPaddingTop))}px`;
	}
...
>
	...
	>
        <button type="button" on:click={close} aria-label="Close" class="absolute top-3 right-3 z-10 p-1 text-gray-500 transition hover:text-red-400"> <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"> <path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12" /> </svg> </button>
        <div class="aspect-square w-full flex-shrink-0 overflow-hidden rounded-t-xl"> {#if product.images && product.images.length > 0} <img src={product.images[0]} alt={product.name} class="h-full w-full object-cover" /> {:else} <div class="flex h-full w-full items-center justify-center bg-gray-800/50"> <span class="text-sm text-gray-500">لا توجد صورة</span> </div> {/if} </div>
        <div class="flex-grow overflow-y-auto p-6 space-y-4">
            <h2 id="product-modal-title" class="mb-1 text-2xl font-semibold text-purple-300 title-glow">{product.name}</h2>
            <p class="mb-3 text-2xl font-bold text-purple-400">{product.price} <span class="text-base font-normal text-gray-400">شيكل</span></p>
            {#if product.description} <p class="mb-4 text-sm text-gray-400">{product.description}</p> {/if}
            {#if isInStock} <div class="space-y-2"> <label class="label !mb-1">اختيار المقاس <span class="text-red-500">*</span></label> <div class="flex flex-wrap gap-2"> {#each availableSizes as size (size)} <button type="button" on:click={() => selectedSize = size} class="rounded-md px-3 py-1.5 text-sm border transition duration-150 ease-in-out" class:bg-purple-700={selectedSize === size} class:text-white={selectedSize === size} class:border-purple-600={selectedSize === size} class:bg-gray-800={selectedSize !== size} class:text-gray-300={selectedSize !== size} class:border-gray-700={selectedSize !== size} class:hover:bg-purple-800={selectedSize !== size} class:hover:border-purple-700={selectedSize !== size}> {size} <span class="text-xs opacity-80">({product.sizes_stock[size]} متوفر)</span> </button> {/each} </div> </div> {#if selectedSize} <div> <label for="quantity" class="label">الكمية (المتوفر: {maxQuantity}) <span class="text-red-500">*</span></label> <input type="number" id="quantity" bind:value={quantity} on:input={handleQuantityChange} min="1" max={maxQuantity} required disabled={maxQuantity < 1} class="input-field w-24 text-center" /> </div> {/if} {:else} <p class="rounded border border-red-600 bg-red-900/50 p-3 text-center text-red-300">عذراً، هذا المنتج غير متوفر حالياً.</p> {/if}
            {#if isInStock && selectedSize} <hr class="my-5 border-gray-700"> <div> <label for="customerName" class="label">الاسم <span class="text-red-500">*</span></label> <input type="text" id="customerName" bind:value={customerName} required class="input-field" placeholder="الاسم الكامل" /> </div> <div> <label for="customerPhone" class="label">رقم الهاتف <span class="text-red-500">*</span></label> <input type="tel" id="customerPhone" bind:value={customerPhone} required class="input-field ltr-input" placeholder="05xxxxxxxx" /> </div> <div> <label for="customerAddress" class="label">العنوان بالتفصيل <span class="text-red-500">*</span></label> <textarea id="customerAddress" bind:value={customerAddress} required rows="3" class="input-field" placeholder="المدينة - الشارع - أقرب معلم"></textarea> </div> {#if error} <p class="rounded border border-red-700 bg-red-900/60 p-3 text-center text-red-300 text-sm"> {error} </p> {/if} {#if successMessage} <p class="rounded border border-green-700 bg-green-900/60 p-3 text-center text-green-300 text-sm"> {successMessage} </p> {/if} <p class="pt-2 text-center text-sm text-yellow-300/80"> <svg xmlns="http://www.w3.org/2000/svg" class="inline h-4 w-4 align-text-bottom ml-1" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M4 4a2 2 0 00-2 2v4a2 2 0 002 2V6h12v6h2V6a2 2 0 00-2-2H4zm0 6a2 2 0 00-2 2v4a2 2 0 002 2h12a2 2 0 002-2v-4a2 2 0 00-2-2H4zm4 4a1 1 0 100-2 1 1 0 000 2zm6 0a1 1 0 100-2 1 1 0 000 2z" clip-rule="evenodd" /></svg> الدفع عند الاستلام </p> <div class="pt-4"> <button type="submit" disabled={submitting || quantity < 1} class="w-full rounded-md bg-gradient-to-r from-green-600 to-green-800 px-5 py-3 text-base font-medium text-white shadow-lg shadow-green-900/30 transition duration-150 ease-in-out hover:scale-[1.02] hover:from-green-500 hover:to-green-700 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2 focus:ring-offset-gray-900 disabled:opacity-60 disabled:cursor-not-allowed glow-effect"> {#if submitting} <span class="animate-pulse">جاري إرسال الطلب...</span> {:else} تأكيد الطلب {/if} </button> </div> {/if}
        </div>
	</form>
</div>
