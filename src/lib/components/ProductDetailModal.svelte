<!-- === Code for src/lib/components/ProductDetailModal.svelte (with Address Field & saving imageUrl) === -->
<script lang="ts">
	import { createEventDispatcher } from 'svelte';
	import type { Product } from '$lib/types';
	import { supabase } from '$lib/supabaseClient'; // To save the order

	// Props
	export let product: Product; // The product to display
	...
	let customerAddress: string = ''; // Address state variable
	let submitting = false;
	let error: string | null = null;
	let successMessage: string | null = null;
	let maxQuantity = 1; // Max quantity based on selected size stock

	// Event Dispatcher
	const dispatch = createEventDispatcher<{ close: {} }>(); // Only need a close event
	...
	async function handleOrderSubmit() {
		...
		if (!customerAddress.trim()) {
			error = 'يرجى إدخال العنوان بالتفصيل.';
			submitting = false;
			return;
		}
		if (!selectedSize) {
			error = 'يرجى اختيار المقاس المطلوب.';
			submitting = false;
			return;
		}
		if (quantity < 1 || quantity > maxQuantity) {
			error = `الكمية المطلوبة غير متوفرة لهذا المقاس (المتوفر: ${maxQuantity}).`;
			submitting = false;
			return;
		}
		// --- End Validations ---
		...
		const orderPayload = {
			...
			order_details: {
				...
				quantity: quantity,
				price: product.price,
				imageUrl: product.images && product.images.length > 0 ? product.images[0] : null // <<< SAVING IMAGE URL HERE
			},
			status: 'new',
		...
		try {
			...
			if (fetchError || !currentProductData) {
				throw new Error('Could not verify product stock before ordering.');
			}
			const currentStock = currentProductData.sizes_stock[selectedSize] ?? 0;
			if (quantity > currentStock) {
				error = `للأسف، نفدت الكمية المطلوبة للمقاس ${selectedSize} أثناء محاولة الطلب. (المتوفر الآن: ${currentStock})`;
				submitting = false;
				return;
			}
			...
			const { error: stockUpdateError } = await supabase
				...
				.eq('id', product.id);
			if (stockUpdateError) {
				console.error('CRITICAL: Order placed but failed to update stock for product', product.id, stockUpdateError);
			} else {
				console.log('Stock updated successfully for product', product.id);
			}
			...
			if (err.message.includes('violates row-level security policy')) {
				error = 'خطأ في الأذونات عند حفظ الطلب. حاول تحديث الصفحة.';
			} else {
				error = 'حدث خطأ غير متوقع أثناء إرسال الطلب: ' + err.message;
			}
		} finally {
			submitting = false;
		}
...
<!-- Modal Overlay -->
<div
	class="fixed inset-0 z-[60] flex items-center justify-center bg-black/80 p-4 backdrop-blur-sm animate-fadeIn"
	on:click|self={close}
	role="dialog" aria-modal="true" aria-labelledby="product-modal-title"
>
	<!-- Modal Content -->
	<div class="relative w-full max-w-lg rounded-xl border border-purple-900/30 bg-gradient-to-br from-gray-950 via-black to-gray-950 shadow-2xl shadow-purple-950/30 max-h-[90vh] overflow-y-auto flex flex-col">
        ...
        <!-- Form Area -->
        <form on:submit|preventDefault={handleOrderSubmit} class="p-6 space-y-4 flex-grow">
                ...
                <div class="flex flex-wrap gap-2">
                    {#each availableSizes as size (size)}
                         <button
                           ...
                           class="rounded-md px-3 py-1.5 text-sm border transition duration-150 ease-in-out"
            ...
            {:else}
                 <p class="rounded border border-red-600 bg-red-900/50 p-3 text-center text-red-300">عذراً، هذا المنتج غير متوفر حالياً.</p>
            {/if}


            <!-- Customer Info (Only if item is in stock and size selected) -->
            {#if isInStock && selectedSize}
                <hr class="border-gray-700 my-5">
                <div>
                    <label for="customerName" class="label">الاسم <span class="text-red-500">*</span></label>
                    <input type="text" id="customerName" bind:value={customerName} required class="input-field" placeholder="الاسم الكامل" />
                </div>
                <div>
                    <label for="customerPhone" class="label">رقم الهاتف <span class="text-red-500">*</span></label>
                    <input type="tel" id="customerPhone" bind:value={customerPhone} required class="input-field ltr-input" placeholder="05xxxxxxxx" />
                </div>
                <!-- ADDRESS FIELD -->
                <div>
                    <label for="customerAddress" class="label">العنوان بالتفصيل <span class="text-red-500">*</span></label>
                    <textarea id="customerAddress" bind:value={customerAddress} required rows="3" class="input-field" placeholder="المدينة - الشارع - أقرب معلم"></textarea>
                </div>

                 <!-- Status Messages within form -->
                 {#if error}
                    <p class="rounded border border-red-700 bg-red-900/60 p-3 text-center text-red-300 text-sm">
                        {error}
                    </p>
                {/if}
                 {#if successMessage}
                    <p class="rounded border border-green-700 bg-green-900/60 p-3 text-center text-green-300 text-sm">
                        {successMessage}
                    </p>
                 {/if}
