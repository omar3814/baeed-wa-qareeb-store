<!-- === Code for src/routes/+layout.svelte (Includes Header, Basket Icon & View) === -->
<script lang="ts">
	// --- IMPORTS ---
	import '../app.css'; // Or '../app.pcss' if using that
	import { supabase } from '$lib/supabaseClient';
	import { onMount, onDestroy } from 'svelte';
	import { authStore } from '$lib/stores/authStore';
	import AdminIcon from '$lib/components/AdminIcon.svelte';   // Floating admin gear icon
	import LoginIcon from '$lib/components/LoginIcon.svelte';   // Floating login user icon
	import BasketIcon from '$lib/components/BasketIcon.svelte'; // Floating basket icon
    import BasketView from '$lib/components/BasketView.svelte'; // Basket modal/view component
	import type { LayoutData } from './$types'; // Type for data from +layout.ts

	// --- PROPS & STATE ---
	export let data: LayoutData; // Receive site settings from +layout.ts load function
	// Reactive variable: update siteSettings when layout data changes
	$: siteSettings = data.siteSettings as Partial<import('$lib/types').SiteSettings & { logo_image_url?: string | null }> || {};

    let showBasket = false; // State to control visibility of the basket modal
    let settingsListener: ReturnType<typeof supabase.channel> | null = null; // For realtime settings updates

	// --- LIFECYCLE ---
	onMount(() => {
		// Get initial session state when component mounts in browser
		supabase.auth.getSession().then(({ data: { session } }) => {
            console.log('[Root Layout] Initial Supabase Session:', session);
			authStore.set(session); // Update global auth store
		}).catch(error => {
        ...
        // Listen for changes to site settings (for header/footer updates)
        if (settingsListener) supabase.removeChannel(settingsListener).catch(console.error); // Clean previous if exists
        settingsListener = supabase.channel('public:site_settings:layout') // Unique channel name
            .on<import('$lib/types').SiteSettings & { logo_image_url?: string | null }>(
                ...
                (payload) => {
                    ...
                    // Update the 'data' prop which triggers the reactive '$: siteSettings' update
...
<div class="min-h-screen bg-black font-sans text-gray-200">
    ...
    <header class="sticky top-0 z-50 border-b border-purple-900/20 bg-black/80 py-3 shadow-lg backdrop-blur-md animate-fadeIn">
        ...
        <div class="container mx-auto flex items-center justify-between px-4">
            ...
            <!-- Right Section (Start in RTL): Login/Admin Icon -->
            <div class="flex basis-1/4 items-center justify-start">
                 <!-- Conditional Login/Admin Icon -->
                {#if $authStore}
                    <AdminIcon /> <!-- Renders small gear button -->
                {:else}
                    <LoginIcon /> <!-- Renders small user button -->
                {/if}
            </div>

            <!-- Center Section: Store Name & Logo Title -->
            <div class="flex basis-1/2 items-center justify-center gap-x-3">
             ...
             <!-- Left Section (End in RTL): Social Icons -->
    ...
    <!-- Basket Icon: Sticky at bottom right -->
    <!-- It receives a function reference (`() => showBasket = true`) -->
    <!-- which it calls internally when its button is clicked -->
    <BasketIcon onOpenBasket={() => showBasket = true}/>

    <!-- Basket View Modal: Rendered conditionally based on showBasket state -->
    <!-- It receives a function reference for its close event -->
    {#if showBasket}
